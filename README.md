...
- 🧩 **浏览器插件**:一键收藏当前网页(自动预填标题/去重提示/私密开关/标签/分类),右键菜单收藏任意链接;若后台配置了 AI,popup 内提供「AI 智能填充」一键补全标题/描述/标签/分类。插件凭后台「安全」页生成的**访问令牌**跨域调用 API(明文仅生成时显示一次,库中只存 SHA-256;改密码自动吊销)
